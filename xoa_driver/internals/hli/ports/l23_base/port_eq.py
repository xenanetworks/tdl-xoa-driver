...
class SerdesEQ:
    def __init__(self, conn: "itf.IConnection", module_id: int, port_id: int, serdes_xindex: int) -> None:
        self.tx_freya = FreyaTxTap(conn, module_id, port_id, serdes_xindex)
        """Freya Tx tap
        """
        self.rx_freya = FreyaRxTap(conn, module_id, port_id, serdes_xindex)
        """Freya Rx tap
        """

        self.tx_thor_loki = PP_PHYTXEQ(conn, module_id, port_id, serdes_xindex)
        """Equalizer settings of the on-board PHY in the TX direction.

        :type: PP_PHYTXEQ
        """

        self.rx_thor_loki = PP_PHYRXEQ(conn, module_id, port_id, serdes_xindex)
