...
class LaserPower:
    ...
    def __init__(self, conn: "itf.IConnection", module_id: int, port_id: int) -> None:
        self.tx = PP_TXLASERPOWER(conn, module_id, port_id)
        """Power of TX laser.

        :type: PP_TXLASERPOWER
        """

        self.rx = PP_RXLASERPOWER(conn, module_id, port_id)
        """Power of RX laser.
...
class PortTransceiver:
    ...
    def __init__(self, conn: "itf.IConnection", module_id: int, port_id: int) -> None:
        ...
        self.rx_los_status = P_TCVRSTATUS(conn, module_id, port_id)
        """L23 port transceiver Rx LOS status information.

        :type: P_TCVRSTATUS
        """

        self.laser_power = LaserPower(conn, module_id, port_id)
        """Transceiver laser power information.
