...
class ThorLinkTrainingSerdes:
    """L23 high-speed port PCS/PMA link training"""

    def __init__(self, conn: "itf.IConnection", module_id: int, port_id: int, serdes_xindex: int) -> None:

        self.status = PP_LINKTRAINSTATUS(conn, module_id, port_id, serdes_xindex)
        """Link training status.
...
class ThorANLT:
    def __init__(self, conn: "itf.IConnection", module_id: int, port_id: int) -> None:
        self.an = ThorAutoNeg(conn, module_id, port_id)
        """Auto-negotiation config and status
        """
        self.lt_config = PP_LINKTRAIN(conn, module_id, port_id)
        """Link Training config and status
        """