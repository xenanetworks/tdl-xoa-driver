...
#####################################################################
    ...
    def port_config_file_header(self, header: Dict[str, Any]) -> str:
        """Generate port configuration file header string from header dictionary

        :param header: dictionary containing file header information
        :type header: Dict[str, Any]
        :return: formatted file header string
        :rtype: str
        """
        header_lines = [
            ";XENAPORT",
            f";FormatVersion: 3",
            f";Savedby: XOA Python API ({header.get('xoa_driver_version', 'n/a')})",
            f";ChassisName: {header.get('chassis_name', 'n/a')}",
            f";ChassisSerial: {header.get('chassis_sn', 'n/a')}",
            f";ChassisVersion: {header.get('chassis_version_str', 'n/a')}",
            f";ModuleName: {header.get('module_name', 'n/a')}",
            f";ModuleModel: {header.get('module_model', 'n/a').replace('_', '')}",
            f";ModuleSerial: {header.get('module_sn', 'n/a')}",
            f";ModuleVersion: {header.get('module_version_str', 'n/a')}",
            f";ModuleRevision: {header.get('module_revision', 'n/a')}",
            f";Port: {header.get('port_id', 'n/a')}",
            ""
        ]
        return "\n".join(header_lines)

    ## Get port configuration
    def get_port_full_config(self, port_ids: Union[List[str], str], file_headers: Union[List[dict], dict]) -> List[str]:
        """Get full config of one or multiple ports

        :param port_ids: list of port ids in format "module_id/port_id"
        :type port_ids: Union[List[str], str]
        :param file_headers: list of file headers for each port. 

        Example of a file header dictionary:
        
        {
            "xoa_driver_version": "1.2.3",
            "chassis_name": "Live Demo Chassis",
            "chassis_sn": "123456",
            "chassis_version_str": "474.0.0+5096.0",
            "module_name": "Odin-1G-3S-6P",
            "module_model": "Z01SODIN",
            "module_sn": "654321",
            "module_version_str": "3.23.0+0.0",
            "module_revision": "Odin-1G-3S-6P[b]",
            "port_id": "1/1"
        }
            
        :type file_headers: Union[List[dict], dict]
        :return: list of full configurations for each port
        :rtype: List[str]
        """
        ...
        if isinstance(file_headers, dict): file_header_list = [file_headers]
        else: file_header_list = file_headers
        for port_id, file_header in zip(port_id_list, file_header_list):
            resp = ''
            ...
            # Remove port id prefix and two spaces from each line to match the xpc format generated by XenaManager GUI.
            resp = resp.replace(f"{port_id}  ", '')
            
            # Remove the <SYNC> tag
            resp = resp.replace('<SYNC>', '')

            # Add P_RESET command at the beginning with file header
            resp = self.port_config_file_header(file_header) + f"P_RESET\n" + resp
    ...
    def config_data_to_command_list(self, config_data: str) -> List[str]:
        """Convert configuration data string to a list of commands.
        ...
        for line in config_data.splitlines():
            if line.startswith(';'):
                ...
                # insert the port id at the beginning of each command
                if port_id:
                    config_data = config_data.replace(line, f"{port_id} {line}")

        # Split the config data into individual commands
        command_list = config_data.strip().split('\n')
        return command_list

    ## Get module configuration
    def get_module_full_config(self, module_ids: Union[List[str], str]) -> str:
        self.send(cmd = f"SYNC ON")
        resp = ''
        if isinstance(module_ids, str): module_list: List[str] = module_ids.split()
        else: module_list = module_ids
        for module in module_list:
            res = self.send(cmd = f"{module} M_FULLCONFIG ?", sync_on=True)
            resp = res.replace('<SYNC>', '')
            resp = resp.strip('\n')
        self.send(cmd = f"SYNC OFF")
        return resp
